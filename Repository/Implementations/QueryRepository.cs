...
namespace Repository.Implementations
{
    public class QueryRepository : IQueryRepository
    {
        ...
        {
            string? rawStatus = statusValue?.ToString()?.Trim();
            ...
            if (Enum.TryParse<QueryStatus>(normalizedStatus, true, out var parsedStatus))
            {
                return parsedStatus;
            }

            Console.WriteLine($"Unknown query status '{rawStatus}'. Defaulting to Open.");
            return QueryStatus.Open;
        }

        private static bool TryParseQueryStatus(string? statusValue, out QueryStatus parsedStatus)
        {
            parsedStatus = QueryStatus.Open;
            if (string.IsNullOrWhiteSpace(statusValue))
            {
                return false;
            }

            string normalizedStatus = statusValue
                .Trim()
                .Replace(" ", string.Empty, StringComparison.Ordinal)
                .Replace("_", string.Empty, StringComparison.Ordinal)
                .Replace("-", string.Empty, StringComparison.Ordinal);

            return Enum.TryParse(normalizedStatus, true, out parsedStatus);
        }

        private static string ToDatabaseStatus(QueryStatus status)
        {
                ...
                using (var cmd = new NpgsqlCommand(qry, _conn))
                {
                    ...
                    while (await reader.ReadAsync())
                    {
                        queries.Add(new Query
                        {
                            ...
                            Status = ParseQueryStatus(reader["c_status"]),
        ...
        public async Task<int> UpdateStatus(int queryid, int empid, string status, string? comment)
        {
            ...
            if (!TryParseQueryStatus(currentStatus, out var currentStatusEnum))
            {
                return 0;
            }

            if (!TryParseQueryStatus(status, out var newStatusEnum))
            {
                return 0;
            }

            if (currentStatusEnum == QueryStatus.Open && newStatusEnum != QueryStatus.InProgress) return 0;
            if (currentStatusEnum == QueryStatus.InProgress && newStatusEnum != QueryStatus.Solved) return 0;
            if (currentStatusEnum == QueryStatus.Solved) return 0;
            ...
            try
            {
                ...
                updateCmd.Parameters.AddWithValue("@c_status", ToDatabaseStatus(newStatusEnum));
